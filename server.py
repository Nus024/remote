...
# Antrean broadcast untuk klien browser PC
client_queues = set()
lock = threading.Lock()
...
def get_active_or_browser_window():
    ...
    if fg:
        ...
        if any(b in t for b in ['brave', 'chrome', 'edge', 'firefox', 'opera', 'youtube', 'tiktok', 'reader']):
            return fg
    ...
    def do_GET(self):
        ...
        if clean_path == '/api/info':
            ...
            info = {
                'ip': LAN_IP,
                'port': PORT,
                'remoteUrl': f'http://{LAN_IP}:{PORT}/remote.html',
                'readerUrl': f'http://localhost:{PORT}/index.html'
            }
            self.wfile.write(json.dumps(info).encode('utf-8'))
            return

        # 2. Server-Sent Events (SSE) Stream untuk Browser PC (Reader)
        elif clean_path == '/api/events' or self.path.startswith('/api/events'):
            self.send_response(200)
        ...
        # 3. File statis biasa (index.html, remote.html, dll.)
        super().do_GET()

    def do_POST(self):
        clean_path = self.path.split('?')[0].rstrip('/')
        if clean_path in ('/api/scroll', '/api/key'):
            content_length = int(self.headers.get('Content-Length', 0))
            body = self.rfile.read(content_length).decode('utf-8')
            try:
                data = json.loads(body)

                # A. Jika ada perintah tombol keyboard khusus
                if 'key' in data:
                    success = press_key(data['key'])
                    self.send_response(200 if success else 400)
                    self.send_header('Content-Type', 'application/json')
                    self.end_headers()
                    self.wfile.write(b'{"status": "ok"}' if success else b'{"error": "invalid key"}')
                    return

                # B. Jika ada perintah swipe video Shorts/TikTok (Pasti 1 video per swipe)
                if 'swipe' in data:
                ...
                # C. Jika ada perintah notch mouse wheel
                if 'notch' in data:
                ...
                # D. Aliran kecepatan kontinu (Continuous velocity untuk Gulir Halus / Kamera)
                velocity = float(data.get('velocity', 0))
                target = data.get('target', 'all')  # 'web', 'windows', atau 'all'
...
def run_server():
    ...
    print("=" * 60)
    print("   SERVER WIRELESS GESTURE BRIDGE AKTIF")
    print("=" * 60)
    print(f"Monitor PC / Laptop : http://localhost:{PORT}/index.html")
    print(f"Kamera HP (Remote)  : http://{LAN_IP}:{PORT}/remote.html")
    print("=" * 60)
    print("Siap menerima kontrol gestur & sentuhan dari HP...")
    print("Mendukung: YouTube Shorts, TikTok, YouTube Video, Web & Dokumen.")
